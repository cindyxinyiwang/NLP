...
from scipy.stats import poisson, nbinom, lognorm, norm, entropy, logistic
...
class lan_plot():
	def __init__(self, filename=None, probs=None, d_list=None):
		# used to store empirical data length distribution
		self.probs = []
		# length distribution of berkerly parser
		#  self.b_prob[0] is the probability of length 0; b_prob[1] is the probability of length 1 etc
	...
	def make_formula_plot(self):
		x = [i for i in range(100)]
		y = [1.1*i*(0.9**i)/100 for i in x]
		self.line_predict, = plt.plot(x, y, color="y")

	def plot(self):
		self.make_real_plot()
		self.make_parser_plot()
		self.make_lognorm_plot()
		self.make_formula_plot()
		...
		plt.legend([self.line_lognorm, self.line_gram, self.real_plt, self.line_neg_binomial, self.line_predict], [ 'lognormal',  'grammar', 'grammar generation', 'Negative Binomial', 'formula prediction'])
