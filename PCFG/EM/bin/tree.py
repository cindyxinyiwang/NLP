...
class Node:
    ...
    def __str__(self):
        if len(self.children) != 0:
            s = "(" + Node.escape(str(self.label))
            for child in self.children:
                s += " " + child.__str__()
            s += ")"
            return s
        else:
            return Node.escape(str(self.label))
...
if __name__ == "__main__":
    ...
    """s = tokenizer.findall(sys.stdin.read())
    i = 0
    while True:
        tree, i = scan_tree(s, i)
    ...
    for line in sys.stdin:
        ...
        e = []
        for w in t.frontier():
            if w.parent.label == '-NONE-':
                e.append(w)
        for w in e:
            w.delete_clean()
        #print t
        #print " ".join(w.label for w in t.frontier())
        print " ".join(w.parent.label for w in t.frontier())
